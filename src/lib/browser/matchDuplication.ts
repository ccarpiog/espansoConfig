 ...
 * doc says what no type can force about it.
 *
 * **Registered since Phase 2d-6-7a.** `MatchDuplicator.svelte` reports a receiver that
 ...
 * and the recovery form; `MatchDuplicator.svelte` does not yet draw the external conflict
 ...
 * installed session's waits forward (the **settled** one's since 2d-6-7a, below), for `rebuiltOver`'s reason in
...
export function reapplyToDiskVersion(
  ...
  const result: MatchDuplicationReapply = {
    kind: 'reapplied',
...
export interface MatchDuplicationView {
   ...
   * and `MatchDuplicator.svelte` reads this list only through `surfaceAcknowledgementOwed` in
