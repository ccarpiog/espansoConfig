...
describe('recovery, per added field', () => {
  it('carries a drafted value, omits an absent-and-blank field, and writes the matching key', () => {
    for (const field of PLAIN_ADDED) {
      ...
      const newMatch = newMatchOfRecovery(transfer, { trigger: ':a', replace: 'b', options: NO_CREATION_OPTIONS }, LITERAL_TRIGGER_ONLY);
      ...
      expect(field in newMatchOfRecovery(none, { trigger: ':a', replace: 'b', options: NO_CREATION_OPTIONS }, LITERAL_TRIGGER_ONLY), field).toBe(false);
    } // End of the loop over the plain added fields
  });
  ...
  it('writes the body under the one content key the draft holds', () => {
    for (const field of CONTENT_FIELDS) {
      ...
      const newMatch = newMatchOfRecovery(transfer, { trigger: ':a', replace: 'the body', options: NO_CREATION_OPTIONS }, LITERAL_TRIGGER_ONLY);
...
describe('the content switch — one compound intention, all or nothing', () => {
  ...
  it('is recovered as a snippet written under the destination key', () => {
    ...
    expect(newMatchOfRecovery(transfer, { trigger: ':a', replace: 'recovered **body**', options: NO_CREATION_OPTIONS }, LITERAL_TRIGGER_ONLY).content).toEqual({
