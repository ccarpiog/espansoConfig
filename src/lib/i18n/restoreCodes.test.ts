 ...
 *
 * The `writeSurfaceOpen` arm appears eight times, once per competing surface,
 * because its key is chosen by the surface it carries and a single value would
 * exercise one of eight sentences. The two external arms (Phase 2d-6-5) render the
...
describe('the restore refusal accessor', () => {
  ...
  it('delegates the open-surface arm rather than carrying a sentence of its own', () => {
    ...
    } // End of the loop over the eight competing surfaces
  });
