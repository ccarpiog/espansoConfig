...
describe('the duplicate panel’s conflict', () => {
  ...
  it('offers a way out that does not claim anything is being edited', async () => {
    ...
    // This panel is about a copy: nobody typed anything, so the raw editor's *Keep
...
describe('the duplicate panel’s refused arm names what this surface drafts', () => {
  /*
  ...
  it('labels the way out by the draft kind, and the same control still dismisses', async () => {
    const panel = mountDuplicator([{ result: REFUSED }]);
    control(panel.target, 'browser.matchDuplication.duplicate').click();
    await settle();
...
}); // End of the "duplicate panel's refused arm" suite

describe('the duplicate panel asks for its outcome to be brought into view', () => {
  /*
  ...
  it('reveals the external panel over a refusal kept as history, and its controls at the reload step (2d-6-7b review, finding 2)', async () => {
    ...
    control(panel.target, 'browser.matchDuplication.duplicate').click();
    await settle();
    expect(panel.target.textContent).toContain(DICTIONARIES.en['browser.matchDuplication.findings']);
    scrolled.length = 0;
...
}); // End of the "duplicate panel asks for its outcome" suite
...
describe('what the duplicate panel says about recovery', () => {
  /*
   * **2c-4c-3b's negative half, on the surface where getting it wrong would cost
   * most.** A duplication drafts a `MatchId`, so there is nothing authored to make a
   * snippet out of — and a recovery create would not be the operation this panel
   * could not carry out either: `DuplicateItem` clones the source item's exact owned
   * runs, comments and line endings included, while a recovery create writes six
   * projected fields. Offering one here as the way to finish a *duplication* is
   * precisely the name the split forbade.
   *
   * `recovery.test.ts` holds the reason at the value level; only a mounted panel can
   * hold that this surface **mounts the shared renderer**, draws no control, and
   * mounts no recovery form.
   */

  /** The reason a duplication's draft kind produces, so a rename is a compile error. */
...
}); // End of the "what the duplicate panel says about recovery" suite

/** The revision an observation of the duplicator's file reads. */
...
/**
 * The duplicator's file as another writer left it: the same two snippets under a
 * new parse, with new arena nodes.
 *
...
function observedFile(revision: ContentRevision = OBSERVED): DocumentView {
  return makeDocument({
    ...
    relativePath: 'match/base.yml',
    revision,
    matches: [item(20, 0, ':sig', revision), item(21, 1, ':date', revision)]
  });
} // End of function observedFile()

/**
 * One narrowed observation of the duplicator's file — Phase 2d-6-7b.
 *
 ...
 * inside it: in Spanish this panel's close control and the operation's *keep
 * editing* choice read identically, as `MatchDeleter.test.ts` explains.
 *
 * @param target - Where the component was mounted.
 * @returns The panel.
 */
...
describe('the duplicate panel under an external conflict, in English and Spanish — Phase 2d-6-7b', () => {
  ...
  it.each(LOCALES)('draws the origin, evidence and comparison, and refuses the duplicate (%s)', (lang) => {
    locale.setOverride(lang);
    const panel = mountDuplicator([{ result: COMMITTED }]);
    expect(controlIn(panel.target, lang, 'browser.matchDuplication.duplicate').disabled).toBe(false);

    panel.deliver(raisedBy(observed(5)));

    const send = controlIn(panel.target, lang, 'browser.matchDuplication.duplicate');
    ...
    expect(panel.target.textContent).not.toContain(
      translate(lang, 'browser.matchDuplication.revisionExpected', { revision: BASE })
    );
    expect(shown).toContain(translate(lang, 'browser.saveOutcome.retainedOperation'));
    expect(shown).toContain(translate(lang, 'browser.saveOutcome.operation.duplicateSnippet'));
  ...
  it.each(LOCALES)('keeps the conflict through Leave this as it is, and resets the reload step (%s)', (lang) => {
    ...
    expect(externalText(panel.target)).toContain(
      translate(lang, 'browser.matchDuplication.reloadIdentifiesNoSnippet')
    );
    ...
    expect(controlIn(panel.target, lang, 'browser.matchDuplication.duplicate').disabled).toBe(true);
    expect(panel.adoptions).toEqual([]);
    expect(panel.closed()).toBe(0);
    panel.stop();
  ...
  it.each(
    ...
    locale.setOverride(lang);
    const panel = mountDuplicator([], { adoption });
  ...
  it.each(LOCALES)('rebuilds over the disk version through Keep what I asked for, and sends only when asked (%s)', async (lang) => {
    ...
    panel.deliver(raisedBy(seen));
    // The window moves to the disk version as the reapply adopts it; this stand-in
    // answers the new projection from here on.
    ...
    expect(panel.calls).toEqual([]);
    controlIn(panel.target, lang, 'browser.matchDuplication.duplicate').click();
    await settle();
    expect(panel.calls).toHaveLength(1);
  ...
  it.each(LOCALES)('withdraws the reload warning when a later reading supersedes the conflict (%s)', (lang) => {
    ...
    expect(offersIn(externalPanel(panel.target), lang, conflictChoiceKey('reloadDiskVersion', 'operationChoice'))).toBe(true);
    const shown = externalText(panel.target) ?? '';
    expect(shown).not.toContain(translate(lang, 'browser.matchDuplication.reloadIdentifiesNoSnippet'));
    expect(shown).toContain(
  ...
  it.each(LOCALES)('refuses the duplicate while a reading waits, and says so once (%s)', (lang) => {
    locale.setOverride(lang);
    const panel = mountDuplicator();
  ...
  it.each(LOCALES)('keeps its session across a change of language (%s)', (lang) => {
    ...
    expect(shown).toContain(translate(other, 'browser.matchDuplication.reloadIdentifiesNoSnippet'));
  ...
  it.each(LOCALES)('names a save as the origin of a save conflict, beside its three revisions (%s)', async (lang) => {
    locale.setOverride(lang);
    const panel = mountDuplicator([{ result: CONFLICTED }]);
    controlIn(panel.target, lang, 'browser.matchDuplication.duplicate').click();
    await settle();
...
}); // End of the "duplicate panel under an external conflict" suite
...
describe('The duplicate panel acknowledges an unknown write outcome on its own panel, in English and Spanish — Phase 2d-6-9b-2', () => {
