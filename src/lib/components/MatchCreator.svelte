<script lang="ts">
   ...
   *
   * **One arm is reachable today.** `matchCreation.ts` offers *Keep editing*
   * alone: *Copy draft* copies a text and this draft is two fields, and *Load the
   * version on disk* would mean re-seeding the destinations from a fresh read,
   * which is conflict capture and preservation — Phase 2c-4a.
   *
   * **What the exhaustive switch forces, and what it does not.** A *new member* of
   * `ConflictChoice` fails to compile here. A *newly offered* member does not: the
   * arms below are drawn as controls the moment the model names one of them, and
