...
The last on-file verdict's one open finding is closed in the current tree. `scan_entries` now limits the guarantee to links the walk observes and states both target behaviours (`crates/espansoconfig-core/src/persist/backup.rs:3533-3543`). The same qualification is present on `observe_entry` (`crates/espansoconfig-core/src/persist/backup.rs:3909-3915`), `open_entry` (`crates/espansoconfig-core/src/persist/backup.rs:3941-3949`), and the recursive walker (`crates/espansoconfig-core/src/persist/backup.rs:3973-3982`). Those statements match the compiled implementations: the macOS helpers are descriptor-relative and use `O_NOFOLLOW` (`crates/espansoconfig-core/src/persist/backup.rs:1637-1641`, `:1670-1689`, `:1699-1718`), while the non-macOS documentation and implementation expressly retain pathname re-resolution and its race (`crates/espansoconfig-core/src/persist/backup.rs:1857-1864`, `:1891-1897`, `:1929-1940`). A shape-based sweep found no remaining unconditional shared API/helper claim that upgrades the macOS result into an all-target guarantee.
