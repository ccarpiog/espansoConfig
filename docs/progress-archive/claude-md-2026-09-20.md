# `CLAUDE.md` as it stood before 2026-09-20 — archived

**History, never an instruction.** On 2026-09-20 the owner removed the project's own review rules
(§7, "Review rounds and when a tail ends") and the phase-by-phase narrative of §6, because §7.1 — *a
fix round that changed at least one source file commissions a review round* — had turned every
blocker fix into a new lettered phase (2d-5-3-A … N, 2d-5-4-A … G) and the workflow's own review rule
already says what to do: one review per phase, blockers fixed, phase closed. The live `CLAUDE.md`
keeps only project facts. This is the full previous text, verbatim, for the record.
