...
## The Next-action prose of Phase 2d-5-3-A — archived 2026-09-05 at Phase 2d-5-3-B

Moved out of `PROGRESS.md`'s live head the moment 2d-5-3-B superseded it, per that file's own
instruction to **archive before writing the record, not after**, and to the target it named a phase in
advance. 103 lines. Nothing here is deleted; two of its sentences are **marked wrong at the top rather
than after it**, which is 2d-5-2b-C's precedent.

> **⚠️ Two sentences below were proved wrong by Phase 2d-5-3-B.** Read them as history.
>
> 1. **"`rg '\.start\(\)' src --glob '!*.test.ts'` finds only `workspace.svelte.ts:3506`."** It finds
>    **two** lines on the tree that closed 2d-5-3-A, because that phase's own fix wrote
>    `` `BrowserState.start()` `` into a comment in `reconciliationCoordinator.ts` — cited here by
>    line as `:979` until Phase 2d-5-3-C found that citation stale in the very commit that wrote it,
>    and now anchored on the comment's own first words, `` `No production code calls `start()` at all
>    today.` ``. The
>    self-invalidating citation `PROGRESS.md` names at its line 21. **The claim it supports is
>    unaffected** — there is still no production caller — and the recipe that survives its own fix is
>    `rg -n '\.start\(\)' src --glob '!*.test.ts' | rg -v '^\S+:[0-9]+: *(//|\*)'`.
>    See `docs/decisions/2d-5-3-B-notes.md` §3.1.
> 2. **"because the batch's `newest_sequence` indexes a queue this session is no longer reading."**
>    True only if the drain reached the Rust session mutex before `WorkspaceSession::open`'s swap
>    block. It is not guaranteed to: the two are serialized in an order neither side chooses, and if
>    `open` wins, the batch describes the **incoming** lifecycle. The refusal is right either way, but
>    the sentence rested on a call order — **the same shape 2d-5-3-A's own finding 1 was raised to
>    remove**. See `docs/decisions/2d-5-3-B-notes.md` §2, and §2.1 for the three further instances of
>    it that the sweep found.
...
## The 2d-5-3-B Next-action prose — archived 2026-09-05 at Phase 2d-5-3-C

Moved out of `PROGRESS.md`'s live head the moment 2d-5-3-C superseded it, per that file's own
instruction to **archive before writing the record, not after**. 115 lines. Nothing here is deleted;
the sentences a later round proved wrong are **marked at the top rather than after them**, which is
2d-5-2b-C's precedent.

> **⚠️ Three claims below were corrected by Phase 2d-5-3-C.** Read them as history.
>
> 1. **The `staleOpen` arm's reason, given here as resting on "the generation alone" because the
>    batch's queue "is either gone or not yet this session's".** There is a **third state** in which
>    it is neither: `open()` bumps the generation unconditionally in its first statement, while
>    `WorkspaceSession::open` returns from `Workspace::discover(root)?` before taking the lock, so a
>    refused `open_workspace` or `list_documents` leaves the **previous** workspace installed and its
>    queue untouched. The refusal stays right; the reason is now unattributability, not absence. See
>    `docs/decisions/2d-5-3-C-notes.md` §1.
> 2. **"`reconciliationCoordinator.ts:979`."** The citation was stale in the very commit that wrote
>    it — the comment stood at `:995` on that tree, and at `:1014` after 2d-5-3-C's own fix. Every
>    instance is now anchored on the comment's opening words,
>    `` `No production code calls `start()` at all today.` ``. The **third** instance of the shape in
>    this chain, not the second. See `2d-5-3-C-notes.md` §2.
> 3. **"The sweep found three more instances of that shape."** A **fourth** stood at
>    `workspaceOpened()`'s gate comment, which claimed Rust holds the workspace being replaced *"only
>    until `WorkspaceSession::open`'s swap block runs"* — it holds it indefinitely when that open is
>    refused. See `2d-5-3-C-notes.md` §1.2.

#### Phase 2d-5-3-B — the round §7.1 commissioned for 2d-5-3-A's fix

**Complete as a round, every gate green — and `SUPERSEDED BY 2d-5-3-C`, never complete.** Risk class
**high**; worker model **opus**. Record:
[`docs/decisions/2d-5-3-B-notes.md`](docs/decisions/2d-5-3-B-notes.md); review
[`docs/reviews/phase-2d-5-3-B.md`](docs/reviews/phase-2d-5-3-B.md). 2d-5-3-A's own Next-action prose is
archived in [`next-action-history.md`](docs/progress-archive/next-action-history.md) under *"archived
2026-09-05 at Phase 2d-5-3-B"*, with the two sentences this round proved wrong marked at the top of
the archived copy rather than after it.

**The verdict was `ship-with-fixes` with 0 blockers**, and the round confirmed the half of 2d-5-3-A
that was right: the *independence* claim — that nothing on the `staleOpen` line observes
`workspaceOpened()`, and that a host may move `openGeneration()` without ever calling it — is true and
survives this phase's rewrite intact.

**Its Medium is that 2d-5-3-A replaced one unstated ordering with another, and that is the same shape
its own finding 1 was raised to remove.** The rewritten `staleOpen` comment claimed *"every identity in
the batch belongs to a workspace lifecycle on its way out"* and nominated as *"what makes the refusal
right on its own"* the sentence *"its `newest_sequence` indexes a queue this session is no longer
reading"*. **Both hold only if the drain reached Rust's session mutex before the open did.** The
reviewer raised this as a claim about the frontend being unable to *know* the order and recorded under
`NOT-VERIFIED` that it had not traced the Rust side; **the orchestrator traced it, which is what turns
the finding from an argument into a measurement**:

- `commands.rs:3491` — `drain_external_changes` runs under `with_workspace_read`, i.e. the session
  mutex.
- `commands.rs:682` — `WorkspaceSession::open` runs `Workspace::discover` *outside* the lock, then
  takes **the same mutex** for one block that calls `reconciliation.begin_epoch(...)` and
  `guard.replace(Open { … })` **together**.

Neither side chooses which reaches it first. Drain first → the batch is the outgoing queue and the
sentence is true. `open`'s swap block first → the queue has already adopted the new epoch and the batch
describes the **incoming** lifecycle, and the sentence is false. **The arm cannot tell**, because the
number that separates two epochs is the batch's own `epoch` — `WorkspaceSession::drain_external_changes`
says so in its own doc — and this arm fires **above** the `staleEpoch` check that reads it. The
*action* is right under both orders, which is why this is a Medium; what was wrong was the
justification. The arm now rests on the generation alone and names both orders, the mutex, and the
check below that could tell them apart.

**The sweep found three more instances of that shape, none in 2d-5-3-A's diff, and all three were
fixed.** `CLAUDE.md` says to sweep for the shape and never for the words of the finding just closed,
and 2c-4a-2 took four passes because each fix left a narrower instance standing. The three are
`runOneDrain()`'s `awaitingReady()` arm, `requestDrain()`'s JSDoc and `workspaceOpened()`'s gate
comment — each asserting that a drain in flight across an open necessarily describes the **outgoing**
workspace. **The scope extension is deliberate and is argued in `2d-5-3-B-notes.md` §2.1**: the fixes
are comment-only, this project's severity standard makes a false source comment its worst class, and
§7.3 makes a correctness defect in source a blocker rather than something a step may close over. Two of
the three keep a justification that never depended on the ordering — `workspaceOpened()`'s real
objection is to accepting **any** batch in that window, because `adopted` has just been cleared — and
the rewrites keep those halves rather than deleting them.

**Its Low is a self-invalidating citation, and the fix is what invalidated it.** `2d-5-3-A-notes.md`
§3.1 justified its finding with `rg '\.start\(\)' src --glob '!*.test.ts'` **matching exactly one
line**. It matches **two**: that phase's own fix wrote `` `BrowserState.start()` `` into a comment at
`reconciliationCoordinator.ts` — the one opening `` `No production code calls `start()` at all
today.` ``, cited by its words because **2d-5-3-C found this very sentence's line citation stale in
the commit that wrote it**. This is the shape line 21 of this file names, and the chain has now
produced it **three** times. **The claim is unaffected and was re-derived** — one match *is* the new comment, the
other is the wrapper's own body at `workspace.svelte.ts:3506`, and `:1891` says *"Created here, started
by nobody"* independently. The recipe that survives its own fix, **run rather than asserted**, returns
the one line: `rg -n '\.start\(\)' src --glob '!*.test.ts' | rg -v '^\S+:[0-9]+: *(//|\*)'`.

**The second half of that Low is an able-to-fail claim that stopped one assertion short.** §3.3 called
the new failing-open case *proven able to fail* on a mutation that fails at `workspace.test.ts:7616` —
the assertion **after the failed open**. The case stops there, so the **third** assertion at `:7624`,
which §3.3 itself calls *what makes the case about the gate*, was never shown able to fail. **Measured
here rather than taken from the reviewer**: reducing `drainMayStart()` to `started && !disposed` fails
the case at **`:7624`** with `expected [ +0, +0 ] to deeply equal [ +0 ]` and `expected 2 to be 1`
beside it. A different line from the mutation above, which is the point.

**No production behaviour changed.** The `reconciliationCoordinator.ts` diff is **comment-only**,
verified mechanically rather than by eye — `git diff -U0` filtered to changed lines that are neither
comments nor blank returns nothing — and `numstat` is `42 23`. No other file under `src/`,
`src-tauri/` or `crates/` changed, which is what the four unmoved gate figures say independently.

**§7.1 commissions a round, so this phase is `SUPERSEDED`, not complete.** The fix changed **one
source file**, `src/lib/browser/reconciliationCoordinator.ts`. The unit is the file and not the line,
and a comment-only change to a source file **is** a source change here — for exactly the reason this
phase and 2d-5-3-A both demonstrate. Under `/autoclaude-opus` a phase gets **one** review invocation
and this phase spent it, so that round is a new corrective phase (`CLAUDE.md` §7.4).

**Nothing is `BLOCKED`.** The one `actionable` item carried from 2d-5-3-A — two `open()` early-return
arms with no workspace-level case — is still a **coverage gap and not a correctness defect in source**,
so §7.3's blocker clause does not apply.

#### The next action is **Phase 2d-5-3-C — the round §7.1 commissioned for 2d-5-3-B's fix**

Scope it to that fix and to nothing else: the **four** rewritten comments in
`src/lib/browser/reconciliationCoordinator.ts` — `runOneDrain()`'s two `staleOpen` arms,
`requestDrain()`'s JSDoc and `workspaceOpened()`'s gate comment — and
`docs/decisions/2d-5-3-B-notes.md` in full, including the two correction blocks it added to
`2d-5-3-A-notes.md` §3.1 and §3.3. **Check the comments against the code, not the code against the
comments**, and note that three of the four are **outside** what §7.1 scoped this round to; §2.1 of the
notes argues the extension and a later round is entitled to judge that trade differently.

Three things worth a reviewer's attention that this round did not reach. **The four comments now assert
a claim about Rust locking that nothing in this repository tests, and nothing can from the frontend** —
it was derived by reading `WorkspaceSession::open` and `with_workspace_read`, so a change to either
would falsify four comments at once with every gate green. **Two of the four keep a justification whose
sufficiency was argued and not tested.** And **2d-5-3's able-to-fail claims for seven of its eight
cases, and its §8.3 five-failure transcript, are still unreproduced** — 2d-5-3-A reproduced three
mutations, this round's reviewer four, this phase one more.
...
## The recorded residues, archived 2026-09-05 at Phase 2d-5-3-C

Moved out of `PROGRESS.md`'s live head at 2d-5-3-C, on **length** rather than on state: the live head
had reached 797 lines against its 800-line hard bound. **None is a correctness defect in source**, so
none held a step open under `CLAUDE.md` §7.3, and none is superseded by the archiving. Item 2 stayed
in the live head as well, because it is a method rule a later step will trip over rather than a
residue. Unedited.

1. **Three 2d-4b residues** — a commit message's own diff figure, a `node_modules` line citation and a
   *"six review rounds"* count in `workspace.test.ts:468` — are in
   [`phase-2d.md`](phase-2d.md). All three are still true.
2. **`scripts/lint/ipc-detail.test.ts` generates its cases from `scannableFiles()`**, so its count moves
   when a file is *added* under the scanned roots and no author touches it. 2d-5-2a moved it by **+2**
   without an author touching it — the fourth recorded instance. **Re-derive a test count per file, on
   a pristine tree, never from the total.**
3. **The mirror's reactivity depends on a writer existing** — `2d-5-2b-notes.md` §14.6 item 1, new at
   2d-5-2b-B. *Recorded only*; three sites assign today.
4. **The detailed Next-action prose of 2d-5-2b and 2d-5-2b-A** — the three open questions 2d-5-2b
...
## The 2d-5-3-E Next-action prose, archived 2026-09-05 at Phase 2d-5-3-F

**Three of its claims were corrected by 2d-5-3-F, and they are marked here — at the top of the archived
copy rather than after it, which is 2d-5-2b-C's precedent.**

1. **"2d-5-3-D's thin item 4 was closed by measurement"** — wrong number. The citation-anchor item is
   2d-5-3-D's §8 item **5**. Its item **4** is the unreproduced able-to-fail residue, which this same
   round's §7 says it cleared **none** of — so one record both closed and left open the same numbered
   item.
2. **"2d-5-3-D's §8 said *five* … and it is still not verified"** — enumerated at 2d-5-3-F, and the
   answer is **six**: `reconciliationCoordinator.ts`'s module doc, the `awaitingWorkspaceReady` doc, three
   paragraphs of the `staleOpen` arm, and `workspace.svelte.ts`'s failed-open arm. 2d-5-3-E's own fix
   added the sixth. **"Tested by none" is false with it** — the workspace half is pinned in Rust, which
   is what 2d-5-3-E's own Medium 1 established.
3. **"never this one"** in the source paragraph this prose describes is true of the batch's
   **provenance** and short by a case under the **property** reading, because `open()` has no
   re-entrancy guard. The refusal is unaffected; the comment now says which reading it means.

**What was *not* wrong**: both of this round's Mediums, their diagnoses, and everything it recorded as
checked and left standing. 2d-5-3-F re-derived all three and they hold.

#### Phase 2d-5-3-E — the round §7.1 commissioned for 2d-5-3-D's fix

**Complete as a round, every gate green — and `SUPERSEDED BY 2d-5-3-F`, never complete.** Risk class
...
[`docs/decisions/2d-5-3-E-notes.md`](docs/decisions/2d-5-3-E-notes.md); review
[`docs/reviews/phase-2d-5-3-E.md`](docs/reviews/phase-2d-5-3-E.md). 2d-5-3-D's Next-action prose —
**115 lines** — is archived in
[`next-action-history.md`](docs/progress-archive/next-action-history.md) under *"archived 2026-09-05 at
Phase 2d-5-3-E"*, with the two claims this round corrected marked at the top of the archived copy
rather than after it.

**Verdict `ship-with-fixes`, 0 blockers**, 2 Medium and 0 Low. **Both were re-derived by the
orchestrator against the code before any fix was applied**, both hold, and both are fixed.

**Both Mediums are claims 2d-5-3-D's own fix introduced, which makes this the fourth consecutive round
of this chain whose whole finding list is its predecessor's fix.** The brief's one instruction —
*check the comments against the code, not the code against the comments* — found both, and neither is
reachable by any gate.

**Medium 1: the replacement written to close a false coverage citation states a false absence, and the
absence is the more dangerous overclaim.** 2d-5-3-D wrote *"Nothing in this repository drives the third
state, and nothing here could … an edit to that early return falsifies this paragraph with every gate
in the project green."* **Re-derived against Rust**: `src-tauri/src/watch_check.rs`'s
`a_failed_reopen_keeps_the_previous_watcher_watching` opens a real tree, refuses a second open with a
path that is not a directory, and then asserts `watch_status()` still answers, `status.epoch == 1`,
`status.ready`, **and a live edit still delivered at epoch 1** — the third state's *workspace* half,
driven and asserted, in a module `src-tauri/src/main.rs` includes under `#[cfg(test)]` with no
`#[ignore]` and no feature gate. So a change letting a refused open replace or empty the session turns
that test **red**, and the "every gate green" sentence is false with it. **Only the queue half is
unpinned** — that test never drains — and that is what the comment now says. **The shape is worth more
than the instance**: a round traded an overclaim about *coverage* for an overclaim about *absence*,
which reads as humility and is exactly as unchecked.

**Medium 2: the paragraph that correctly removed a case from the third state then attributed it to the
wrong one of the remaining two.** 2d-5-3-D wrote that a refused `list_documents` *"is the
incoming-lifecycle case above with its queue reset to empty"*. It establishes only that
`open_workspace` **succeeded**. The arm above separates its first two states by the **lock race** —
*win, and the batch is the outgoing queue; lose, and `open`'s swap block has already run* — and
`drain_external_changes` reaches that same session mutex through `with_workspace_read`. Whether the
in-flight drain was serviced before or after the swap is what decides which state the batch is in, and
a refusal observed later in `open()` is no evidence either way. **It is one of the first two, whichever
the race gave.** Right that it is not the third state; wrong that it is the second.

**What the round checked and left standing**, each re-derived: `awaitingReady()`'s new premise is true
at its arm; `Workspace::discover(root)?` is outside the lock; `begin_epoch` and `guard.replace` are one
block and `guard.replace` is the slot's only writer; `workspace.test.ts`'s failed-open case pins the
*gate* and holds `drainSequences` at `[0]`; no `pub async fn` in `commands.rs`. **And it closed
2d-5-3-D's thin item 5 by measurement**: all five anchored citations still resolve after the rewording
that prompted the worry.

**§7.1 commissions a round, so this phase is `SUPERSEDED`, not complete.** The fix changed **one
source file** — `src/lib/browser/reconciliationCoordinator.ts`, `numstat` **`23 10`**. The diff is
**comment-only, proven mechanically rather than by eye** (`git diff -U0` filtered to changed lines that
are neither comments nor blank returns nothing), and **the unit is the file and not the line**. The
record half of the sweep — the correction block at the top of `2d-5-3-D-notes.md`, the nested
correction in `2d-5-3-C-notes.md` §1, the marker in `next-action-history.md` and this file — commissions
nothing, because every one of those is on §7's closed list. Under `/autoclaude-opus` a phase gets **one**
review invocation and this phase spent it, so that round is a new corrective phase (`CLAUDE.md` §7.4).

**Nothing is `BLOCKED`.** The review marked both Mediums *actionable* correctness defects in source —
blockers under §7.3 unless fixed in this round's fix — and **both were fixed in it**. No other item
names a defect in a source file.

#### The next action is **Phase 2d-5-3-F — the round §7.1 commissioned for 2d-5-3-E's fix**

Scope it to that fix and to nothing else: the **two** rewritten paragraphs in
`src/lib/browser/reconciliationCoordinator.ts` — `runOneDrain()`'s `staleOpen` arm, specifically the
`list_documents` paragraph and the paragraph about what pins the third state — and
`docs/decisions/2d-5-3-E-notes.md` in full, including the correction block it added at the top of
`2d-5-3-D-notes.md`, the nested correction in `2d-5-3-C-notes.md` §1 and the marker block in
`next-action-history.md`. **Check the comments against the code, not the code against the comments** —
that instruction has now found nine Mediums across five rounds and is worth repeating verbatim.

**Four things worth a reviewer's attention that this round did not reach**, all four marked in
`2d-5-3-E-notes.md` §8. **The new Rust citation is this round's own most likely defect**: the comment
now names `a_failed_reopen_keeps_the_previous_watcher_watching` by symbol, and nothing in this
repository fails if that test is renamed, moved or deleted — the same class as the citation drift
recorded below, introduced deliberately because the alternative was leaving a false absence claim
standing. **The queue half remains unpinned and is now the only unpinned half**, so an edit that reset
the reconciliation queue on the refusal path still falsifies the paragraph with every gate green; that
is narrower than what 2d-5-3-D claimed and it is still real. **The three-state claim is asserted across
several comment paragraphs and this round did not enumerate them** — 2d-5-3-D's §8 said *five* and its
reviewer recorded that as `NOT-VERIFIED`. **2d-5-3-F enumerated it: the answer is six**, and 2d-5-3-E's
own fix added the sixth; *"tested by none"* is false with it, because the workspace half is pinned in
Rust. **And 2d-5-3's
able-to-fail claims for seven of its eight cases, and its §8.3 five-failure transcript, are still
unreproduced** — no round of this chain has cleared that residue.

**One thing a reviewer should judge rather than inherit, carried forward and now half-discharged.**
Five positions cite a comment by **the words it opens with** or a function by **symbol** rather than by
line. **An anchor is robust against renumbering, not against a rewording** — 2d-5-3-D reworded two of
the anchored comments, and **this round checked all five and found they still resolve**, which closes
the specific worry and not the class. This round then **added a sixth anchor of a new kind** — a Rust
test named by symbol from a TypeScript comment — which nothing enforces either. The durable
...
## Phase 2d-5-3-H's record and its Next-action prose — archived 2026-09-05 at Phase 2d-5-3-I

**123 lines, moved verbatim out of `PROGRESS.md`'s *Next action* when 2d-5-3-I superseded them.**
History, never an instruction: the only live next action is the one in `PROGRESS.md`.

**Corrections 2d-5-3-I made to the text below are marked here, at the top rather than after it**,
which is 2d-5-2b-C's precedent — a reader who scrolls stops at the first wrong sentence.

- **The archive count in the block below is short by one.** It says two archives were taken before a
  word of 2d-5-3-H's record was written. That is true of the two taken *first*, and a **third**
  followed after the record went in — that round's header's own headroom narrative, 80 lines, to
  [`phase-2d.md`](phase-2d.md). 2d-5-3-H's own `PROGRESS.md` header said three and its git-state row
  said two; 2d-5-3-I found the pair by counting one against the other and corrected the row in place.
- **The Next-action block below scoped 2d-5-3-I to "the **two** rewritten passages"**, and the round
  it scoped found a third site of one passage's own defect — a fourth `the paragraph above` at what
  was then `:824`, outside both passages and inside the same comment block. The scope sentence was not
  wrong about what §7.1 commissioned; it is recorded here because **a scope written as a count of
  passages invites a sweep bounded by that count**, and the shape being swept for was not.

#### Phase 2d-5-3-H — the round §7.1 commissioned for 2d-5-3-G's fix

**Complete as a round, every gate green — and `SUPERSEDED BY 2d-5-3-I`, never complete.** Risk class
**high**; worker model **opus** (no implementation worker: the phase's product is a review and its fix,
both taken by the orchestrator). Record:
[`docs/decisions/2d-5-3-H-notes.md`](docs/decisions/2d-5-3-H-notes.md); review
[`docs/reviews/phase-2d-5-3-H.md`](docs/reviews/phase-2d-5-3-H.md). 2d-5-3-G's record and Next-action
prose — **112 lines** — are archived in
[`next-action-history.md`](docs/progress-archive/next-action-history.md) under *"archived 2026-09-05 at
Phase 2d-5-3-H"*, and that round's **verification narrative** — **71 lines** — in
[`phase-2d.md`](docs/progress-archive/phase-2d.md). **Two archives were taken before a word of this
record was written**, which is what the header demanded and what the arithmetic below is measured on.

**Verdict `ship-with-fixes`, 0 blockers**, **5 SHOULD-FIX — 2 in source, 3 in the record**. **All five
were re-derived against the code before any fix was applied**, and all five hold.

**This round found a real defect in the previous round's fix**, which makes it 2d-5-3-G's stated *six of
seven* plus one — **seven of the eight rounds `2d-5-3-A` … `2d-5-3-H`**, the only figure here whose
derivation is written beside it.

**The round's own nominated most likely defect was the wrong one, and the right one sat on top of it.**
2d-5-3-G nominated its first reading of the `WorkspaceSession` struct — the Rust six previous rounds had
reasoned around. **That reading was re-derived here and holds**: `Open` carries `workspace`, `backups`
and `watcher`; `reconciliation` sits beside the session; its doc says the queue is *"emptied by a
replacement rather than replaced by one"*. What broke was the **prose built on the correct reading**.

**Source finding 1: the replacement claim is true only at an instant and was written as true now.** *"In
case 2 the batch already **is** the incoming lifecycle's queue and Rust is still holding that lifecycle
… satisfied there outright"* is evaluated **after the await**, and nothing gates `open()` on the drain —
a further **successful** open installs another lifecycle and `begin_epoch` **empties** the one
session-long queue, which is the very Rust the previous round established. The overlap is **driven**:
`workspace.test.ts`'s *"lets the newer open win, however late the older one answers"*, in a suite named
**overlapping requests**, runs two — the same test 2d-5-3-G's own Medium 3 used against an absence
claim, one round earlier. **The clause this round deleted was the only one carrying the time index**:
2d-5-3-F's *"a **later** open refusing at `Workspace::discover(root)?`"* really was load-bearing on
nothing, and it was also the thing placing the property at a **moment**. That is the lesson rather than
a case for restoring it — **a removal has to check what else the removed clause was carrying.** The
paragraph's conclusion is unaffected, because it needs the property to hold only at *some* moment in
case 2.

**Source finding 5 (Low): a citation over-claiming one of its two referents, and a newly ambiguous
*"the paragraph above"*.** The paragraph below really does rest the refusal on unattributability; the
one opening *"Which lifecycle the batch describes"* says only that the lifecycle *"is not knowable here,
and the refusal does not need it to be"* — **weaker**, so *"both say"* is false of the pair. And the
case-2 sentence this round wrote became a **second** site asserting the property, so *"the paragraph
above"* acquired two referents in this round's own rewrite. Both fixed by naming the sites by their
opening words.

**Record finding 2: the section removed one unverified absence claim and asserted another in the next
clause.** `2d-5-3-G-notes.md` §2 dropped *"`open()` has no re-entrancy guard"* **for being unverified**
and then wrote *"nothing has replaced that lifecycle since"*. **Same absence, opposite sign, no more
evidence** — and finding 1 shows the second is the false one. **An absence claim is not made safe by
being the conclusion of a correction.**

**Record finding 3 (Low): the section that retired the count asserts the count.** §4 writes *"this round
gets **eight**"* four lines above *"**No number is written down as the answer**"* and above *"Writing
"eight" would be the sixth instance"*. The correction block it produced on `2d-5-3-E-notes.md` §7
carries the identical pair. **The ruling is the sound half and it stands** — the criterion problem is
the finding — but **a figure offered as an illustration of an ambiguous predicate is still a figure the
next round inherits**, and this one was inherited inside the same commit. **Neither number is re-derived
here**, on the ruling itself.

**Record finding 4 (Low): three further uncounted counts, one demonstrably underived.** *"The **fifth**
time"*, *"a **sixth** opening-words anchor"* and *"nominated a citation checker **five** times"*. The
third is checkable and fails: the archived Next-action blocks of **2d-5-3-E** and **2d-5-3-F** *both*
say *"four times"* and nothing between them records a further nomination. The round's sweep looked for
*"three paragraphs"* — **the words of its own Low 2** — rather than for the shape, *a bare ordinal or
cardinal asserted about this chain's own history*.

**§7.1 commissions a round, so this phase is `SUPERSEDED`, not complete.** The fix changed **one source
file** — `src/lib/browser/reconciliationCoordinator.ts` — comment-only, proven mechanically. The other
three fixes are record-only.

**Nothing is `BLOCKED`.** No item in `2d-5-3-H-notes.md` §7 names an unfixed correctness defect in a
source file.

#### The next action is **Phase 2d-5-3-I — the round §7.1 commissioned for 2d-5-3-H's fix**

Scope it to that fix and to nothing else: the **two** rewritten passages in
`src/lib/browser/reconciliationCoordinator.ts` — the case-2 sentence, which now carries a time index and
names what the arm does not observe, and the queue-half sentence, which now names both asserting sites
by their opening words and splits the two justification paragraphs — together with
`docs/decisions/2d-5-3-H-notes.md` in full and the four correction blocks this round added to
`2d-5-3-G-notes.md` (three) and `2d-5-3-E-notes.md` (one). **Check the comments against the code, not
the code against the comments** — the instruction that found both of this round's source findings, as
it has found the substantive finding of every round of this tail. **No tally of them is written here**,
per finding 4 below: a bare cardinal about this chain's own history is the shape that round asserted
and this one had to correct.

**What this round did not reach is marked item by item in `2d-5-3-H-notes.md` §7**, each with its
§7.3 mark. **The sharpest is this round's own item 7**: §5's fix asserts that *"the refusal
does not need it to be [knowable]"* is **weaker** than *"the refusal rests on unattributability"*. That
is a reading of two English sentences, not of code; **no gate and no test can separate them**, and a
later round deciding they say the same thing would make the fix over-precise rather than wrong — but it
would be the third consecutive round to correct that one sentence. **The new time index is prose and
nothing pins it** — a later round dropping the qualifier again turns nothing red, and a dropped
qualifier is precisely the class this tail keeps finding. **The overlapping-open case still drives no
Rust** (`scriptedCommands()`), so the new citation pins the frontend's behaviour and must not be read as
Rust coverage. **`commands.rs` beyond the three items this tail has read is still unread**, unchanged —
this round read no new Rust. **2d-5-3's able-to-fail residue (2d-5-3-D §8 item 4) is still unreproduced
after five consecutive rounds**; the honest statement is that no round has had it in scope, and scope is
set by §7.1 rather than chosen. **The citation checker is still unbuilt and this round states no
nomination figure**, per finding 4 — two archived blocks say *"four times"*, one says *"five"*, and
nothing derives either.

**Do not read a closure prediction into this block, because there is none.** The header made one for
five rounds running and was wrong every time, always in the same direction; this record makes none. What
the mechanism guarantees is narrower and is all that is worth carrying: **the first fix round of this
tail that changes no source file is its last**, and nothing in this round's diff says whether the next
one will be that.
...
## Phase 2d-5-3-J's record and its Next-action prose — archived 2026-09-05 at Phase 2d-5-3-K

**Two claims in the blocks below are corrected by 2d-5-3-K and the correction travels with
them.** *"with `workspace.svelte.ts:3502` the only route to the coordinator"* is **false** —
`start()` is the only route to a *drain*, and coordinator code runs in that test throughout
(`createBrowserState` constructs one, every `open()` calls `workspaceOpened()` before its first
await, and the winning open reaches `workspaceReady()`); the conclusion *no drain is issued*
holds on the corrected reason. And `reconciliationCoordinator.test.ts:750` names a line one below
the `it(...)` it cites, which is on **749**. `docs/decisions/2d-5-3-J-notes.md` carries both
corrections as blocks at their sites.
