...
//!   pins one shape per code, and a unit variant among the seventy-three would make
...
/// single unit variant among the seventy-three would make the pinned `'object'`
...
fn every_draft_error_variant_crosses_as_an_object() {
    ...
    assert_eq!(
        declared.len(),
        73,
        "DraftError declares 73 refusals since Phase 4-4: {declared:?}"
    );
    assert!(
        bare.is_empty(),
