...
use crate::draft::match_draft::{MatchField, SequenceField};
...
/// - a scalar edit may name `<match>.<scalar key>` or
///   `<match>.<triggers|search_terms>[i]`, and nothing else. The second form is
///   a scalar-node replacement at a position that already exists, which is why
///   it is not a cardinality change;
...
///   new sequence item and an insertion into `<match>.vars` a new mapping
///   entry under a key no schema fixes; both are refused here. That an
///   insertion's *value* is always a scalar needs no check — a
///   [`crate::patch::FieldInsert`] carries a `String` and renders it through
///   [`crate::emit::choose_scalar`], so there is no spelling of it that builds a
///   collection;
/// - a removal may only name `<match>.<scalar key>`.
///
...
/// 3. a key the batch names that the mapping writes more than once;
...
pub fn check_batch_independence(
    ...
    check_every_named_key_is_unique(mapping, original_keys, edits)?;
...
/// Check 3: every key the batch names is written once in the original mapping.
///
/// A path names the **first** entry with a given key
/// (`crate::patch::path::resolve`'s rule), so a batch that names a repeated key
/// addresses one occurrence and reads as though it addressed the other.
...
fn key_in<'a>(mapping: &DocumentPath, path: &'a DocumentPath) -> Option<&'a str> {
    ...
    segments[base.len()].as_key()
} // End of function key_in()

/// Whether `path` names a scalar of the closed surface: a schema-known scalar
/// field, or an existing element of one of the two string sequences.
fn names_a_surface_scalar(mapping: &DocumentPath, path: &DocumentPath) -> bool {
    let base = mapping.segments().len();
    let segments = path.segments();
    let Some(key) = key_in(mapping, path) else {
        return false;
    };
    match segments.len() - base {
        1 => MatchField::from_key(key).is_some(),
        2 => {
            SequenceField::from_key(key).is_some()
                && matches!(segments[base + 1], PathSegment::Index(_))
        }
        _ => false,
    }
} // End of function names_a_surface_scalar()

/// Whether `path` names a schema-known scalar **entry** of the match mapping.
fn names_a_surface_field(mapping: &DocumentPath, path: &DocumentPath) -> bool {
    let base = mapping.segments().len();
    path.segments().len() == base + 1
        && key_in(mapping, path)
            .and_then(MatchField::from_key)
            .is_some()
}
